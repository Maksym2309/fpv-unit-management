...
const BANDS = {
  ...
  // Окремий бенд, щоб частота була видна на карті й не плуталась зі
  // стандартним B5, яким користуються решта бортів.
...
// Email адміністратора (може завершувати будь-який виліт) — задається
// в Config.gs. Тут його більше немає навмисно: Config вставляється один
// раз, і оновлення цього файлу не затирає твоїх значень.
...
//
// НАЛАШТУВАННЯ: ID спільної теки задається в Config.gs (INFO_ROOT_ID) —
...
//
// НАЛАШТУВАННЯ — у Config.gs (обидва рядки, інакше застосунок тихо
// працює повільним шляхом):
